...
// Supported versions please add new ones here, never remove old ones
...
// Master key derivation from wallet signer gets you per-capsule signature isolation
export async function deriveMasterKeyFromAddress(
    ...
    const salt = ethers.getBytes(
        ethers.keccak256(
            ethers.solidityPacked(["string", "address"], ["TimeCapsule-v3", CONTRACT_ADDRESS])
        )
    );
...
export async function encryptForWallet(
    ...
    const masterKey = await deriveMasterKeyFromAddress(signer, capsuleNonce, issuedAt, expiresAt);
    ...
    try {
        const dataKey = await crypto.subtle.importKey("raw", dataKeyRaw, "AES-GCM", false, [
            "encrypt",
            "decrypt"
        ]);
...
export async function wrapWithDrandTimelock(
    ...
    try {
        ...
        if (errorMsg.includes("timed out")) {
            ...
            userMessage =
                "Network error connecting to time-lock service - please check your connection";
...
export async function unwrapDrandTimelock(payload: CapsulePayload): Promise<CapsulePayload> {
    try {
        ...
        if (errorMsg.includes("round not available") || errorMsg.includes("UNAVAILABLE")) {
            throw new Error(
        ...
        if (errorMsg.includes("malformed")) {
            throw new Error("Capsule data is corrupted or was modified. Unable to decrypt.");
        }

        throw new Error(`Failed to decrypt time-locked capsule: ${errorMsg}`);
    }
}
